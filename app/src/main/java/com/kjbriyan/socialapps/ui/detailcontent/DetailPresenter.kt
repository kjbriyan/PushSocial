...
class DetailPresenter(private val mView: DetailView) {
        ...
        Initretrofit().getInstance().getbtnlike(usrname)
            .enqueue(object : Callback<ResponseLiked> {
                override fun onResponse(
                    ...
                    response: Response<ResponseLiked>
                ) {
                    val res = response.body()
                    mView.onLikeCon(res?.data)
                    mView.onHideloading()

                }

                override fun onFailure(call: Call<ResponseLiked>, t: Throwable) {
                    mView.onDataeror(t)
                    mView.onHideloading()
                }
    ...
    fun btnlike(usrname: String, liked: String, idpost: String) {
        mView.onLoading()
        Initretrofit().getInstance().postlike(usrname, liked, idpost)
